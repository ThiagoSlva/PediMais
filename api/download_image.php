...
// ===== Proteção SSRF: Bloquear IPs internos/privados =====
if (filter_var($image_url, FILTER_VALIDATE_URL)) {
    ...
    if (
    $host === 'localhost' ||
    str_starts_with($resolved_ip, '127.') ||
    str_starts_with($resolved_ip, '10.') ||
    str_starts_with($resolved_ip, '192.168.') ||
    str_starts_with($resolved_ip, '169.254.') ||
    str_starts_with($resolved_ip, '172.') ||
    $resolved_ip === '0.0.0.0' ||
    str_starts_with($resolved_ip, '::') ||
    !filter_var($resolved_ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)
    ) {
        echo json_encode(['success' => false, 'error' => 'URL não permitida']);
        exit;
    }
    ...
    if (!in_array($scheme, ['http', 'https'])) {
        echo json_encode(['success' => false, 'error' => 'Protocolo não permitido']);
        exit;
    }
}
...
try {
    ...
    if (strpos($image_url, 'data:image/') === 0) {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_MAXFILESIZE => 10 * 1024 * 1024, // Limite 10MB
        ]);
